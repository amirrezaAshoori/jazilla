...
const PostQuestionModal = () => {
  ...
  const { price } = usePrice({
    amount: postQuestionState?.price,
    currencyCode: "USD",
  });
    ...
    <div className="p-6 flex flex-col">
      <div className="flex items-center space-x-4 bg-gray-100 p-3">
        <div className="max-w-[90px] w-full grid place-items-center bg-white">
          <Image
            src={postQuestionState?.image?.img_url}
            alt={postQuestionState?.name}
            height={70}
            width={70}
          />
        </div>
        <div>
          <h1 className="text-lg text-gray-800 font-semibold">
            {postQuestionState?.name}
          </h1>
          <h1 className="text-sm text-gray-500 dark:text-gray-700">
            {price}
          </h1>
        </div>
      </div>
